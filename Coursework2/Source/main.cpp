...
#pragma region 1. Initialise application
	...
	try
	{
		...
		// 1.4 Create main application scene object (singleton)
		mainScene = Scene::CreateScene(600, 600, L"DirectX 11", L"DirectX 11", nCmdShow, hInstance, WndProc);

		if (!mainScene)
			throw exception("Cannot create main application scene");
	}
	catch(exception& e)
	{
		cout << e.what() << endl;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		// Bind HWND to host Scene - this allows all relevant app data to be accessed when processing events related to the HWND.
	...
	case WM_CLOSE:
	{
		// Window will close - decouple HWND from main Scene
		Scene *mainScene = (Scene*)GetWindowLongPtrW(hWnd, GWLP_USERDATA);
